...
PostgreSQL accessed through GORM, organized into schemas: `auth`,
...
select-only. The GORM callback sets `app.current_user_id` per request,
so you can tighten a policy to owner-only on any table you add.
