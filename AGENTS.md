...
background workers. Every layer is designed for gRPC from the ground up.
...
internal/
  grpcsvc/      thin gRPC handlers: proto <-> domain mapping
  interceptor/  unary interceptors (recovery, logging, locale,
                error-map, request-id, auth)
  <domain>/     business logic per area (auth, users, settings,
                suspension, notifications); entities live here too
